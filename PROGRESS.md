...
- Looks 已完成公式化灵感复刻：支持上传灵感图或图片链接，输出色彩/轮廓/叠穿/视觉中心公式、关键单品、衣橱借用与替代建议，以及“我的版本怎么穿”。
- Travel 已完成扩展打包方案：支持目的地、天数、场景生成旅行清单、按天轮换建议、鞋履/包袋/外层缺口提示，以及最近方案保存/重开/更新/删除。
...
- 已从“按类别匹配单品”升级为“按穿搭公式匹配”：AI 拆出色彩、轮廓、叠穿和视觉中心公式；关键单品记录 slot、轮廓、层次角色、重要度和替代方案；衣橱匹配会综合类别、slot、颜色、轮廓、风格标签和层次角色，并在没有同类单品时给替代建议。
